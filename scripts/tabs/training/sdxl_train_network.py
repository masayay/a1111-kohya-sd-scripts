...
def create_ui():
    ...
    # train_networkから引数を読み込む
    templates, _ = load_args_template("train_network.py")
    script_file = os.path.join(ROOT_DIR, "kohya_ss", "sdxl_train_network.py")
    ...
    with gr.Column():
        ...
        with gr.Box():
            with gr.Row():
                init_id = presets.create_ui("sdxl_train_network", get_templates, get_options)
        with gr.Row():
            with gr.Group():
                with gr.Box():
